...
class GMBProfileService {
  ...
  async getBusinessProfile(locationName: string): Promise<GMBApiResponse> {
    try {
      const response = await fetch(
        `${this.baseUrl}/${encodeURIComponent(locationName)}/details`,
        {
          method: "GET",
  ...
  ): Promise<GMBApiResponse> {
    try {
      const response = await fetch(
        `${this.baseUrl}/${encodeURIComponent(locationName)}/update`,
        {
          method: "PUT",
          headers: this.getHeaders(),
  ...
  async patchBusinessProfile(
    ...
    try {
      const response = await fetch(
        `${this.baseUrl}/${encodeURIComponent(locationName)}/patch`,
        {
          method: "PATCH",
          headers: this.getHeaders(),
      ...
      return {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to patch business profile",
      };
    }
  }
  ...
  async getBusinessReviews(locationName: string): Promise<GMBApiResponse> {
    try {
      const response = await fetch(
        `${this.baseUrl}/${encodeURIComponent(locationName)}/reviews`,
        {
          method: "GET",
          headers: this.getHeaders(),
        },
  ...
  async getBusinessPhotos(locationName: string): Promise<GMBApiResponse> {
    try {
      const response = await fetch(
        `${this.baseUrl}/${encodeURIComponent(locationName)}/photos`,
        {
          method: "GET",
          headers: this.getHeaders(),
        },
      ...
      return {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to fetch business photos",
      };
    }
  }
  ...
  async getBusinessPosts(locationName: string): Promise<GMBApiResponse> {
    try {
      const response = await fetch(
        `${this.baseUrl}/${encodeURIComponent(locationName)}/posts`,
        {
          method: "GET",
          headers: this.getHeaders(),
        },
      ...
      return {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to fetch business posts",
      };
    }
  }
   ...
   */
  validateProfile(profile: BusinessProfile): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    if (
      profile.phoneNumbers.primary &&
      !/^\+?[\d\s\-\(\)]+$/.test(profile.phoneNumbers.primary)
    ) {
  ...
  transformApiDataToProfile(apiData: any): BusinessProfile {
    return {
      ...
      serviceArea: {
        businessType:
          apiData.serviceArea?.businessType || "CUSTOMER_LOCATION_ONLY",
      ...
      accessibility: {
        ...
        wheelchairAccessibleParking:
          apiData.attributes?.wheelchairAccessibleParking || false,
        wheelchairAccessibleRestroom:
          apiData.attributes?.wheelchairAccessibleRestroom || false,
        wheelchairAccessibleSeating:
          apiData.attributes?.wheelchairAccessibleSeating || false,
      },
      amenities: {
