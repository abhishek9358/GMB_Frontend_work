...
function MetricCard({ label, mobile, desktop, unit, mobileScore, desktopScore, isLoading }: any) {
  // Use scores to determine color (score ranges from 0 to 1)
    ...
    <div className="p-4 rounded-lg bg-gray-50 border border-gray-200">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <p className={`text-xl font-bold ${getColor(mobileScore)}`}>
            {mobile}
            {unit}
          </p>
          <span className={`text-xs px-2 py-0.5 rounded-full ${getScoreBadgeColor(mobileScore)}`}>
            {getScoreLabel(mobileScore)}
          </span>
        </div>
        <div>
          <p className="text-xs text-gray-500 mb-1">Desktop</p>
          <p className={`text-xl font-bold ${getColor(desktopScore)}`}>
            {desktop}
            {unit}
          </p>
          <span className={`text-xs px-2 py-0.5 rounded-full ${getScoreBadgeColor(desktopScore)}`}>
            {getScoreLabel(desktopScore)}
          </span>
        </div>
      </div>
    </div>
...
export default function SearchConsoleOverview() {
  ...
  const [indexingData, setIndexingData] = useState<any>(null);
  const [coreWebVitals, setCoreWebVitals] = useState<any>(null);
  const [isCoreWebVitalsLoading, setIsCoreWebVitalsLoading] = useState(false);
  const [error, setError] = useState<string>("");
  ...
  async function fetchCoreWebVitals(siteUrl: string) {
    ...
    setIsCoreWebVitalsLoading(true);
    setCoreWebVitals(null); // Clear previous data
    ...
    try {
      ...
      setCoreWebVitals({ error: error.response?.data?.detail || "Failed to load Core Web Vitals" });
    } finally {
      setIsCoreWebVitalsLoading(false);
    }
  }
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                {isCoreWebVitalsLoading ? (
                  <div className="text-center py-8 text-gray-500">
                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto mb-2"></div>
                    <p className="text-sm">Loading Core Web Vitals...</p>
                  </div>
                ) : coreWebVitals && !coreWebVitals.error ? (
                  <div className="space-y-4">
                    {/* Performance Scores */}
                    <div className="space-y-2 pb-4 border-b">
                      ...
                      <PerformanceScore 
                        score={coreWebVitals.mobile?.performanceScore || 0} 
                        label="📱 Mobile" 
                      />
                      <PerformanceScore 
                        score={coreWebVitals.desktop?.performanceScore || 0} 
                        label="🖥️ Desktop" 
                      />
                    </div>

                    {/* Field Data Distribution */}
                    {coreWebVitals.mobile?.total > 0 && (
                      <div className="grid grid-cols-2 gap-4">
                        {/* Mobile */}
                        <div>
                          <p className="text-sm font-medium text-gray-700 mb-3 flex items-center gap-1">
                          ...
                          <div className="space-y-2">
                            <div className="flex justify-between text-xs items-center">
                              ...
                              <span className="font-semibold">
                                {coreWebVitals.mobile.goodPercentage?.toFixed(1)}%
                              </span>
                            </div>
                            <div className="w-full bg-gray-200 rounded-full h-2">
                              <div
                            ...
                            <div className="flex justify-between text-xs items-center">
                              ...
                              <span className="font-semibold">
                                {coreWebVitals.mobile.needsImprovementPercentage?.toFixed(1)}%
                              </span>
                            </div>
                            <div className="w-full bg-gray-200 rounded-full h-2">
                              <div
                            ...
                            <div className="flex justify-between text-xs items-center">
                              <span className="text-red-700">Poor</span>
                              <span className="font-semibold">
                                {coreWebVitals.mobile.poorPercentage?.toFixed(1)}%
                              </span>
                            </div>
                            <div className="w-full bg-gray-200 rounded-full h-2">
                              <div
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            <div className="flex justify-between text-xs items-center">
                              ...
                              <span className="font-semibold">
                                {coreWebVitals.desktop.goodPercentage?.toFixed(1)}%
                              </span>
                            </div>
                            <div className="w-full bg-gray-200 rounded-full h-2">
                              <div
                            ...
                            <div className="flex justify-between text-xs items-center">
                              ...
                              <span className="font-semibold">
                                {coreWebVitals.desktop.needsImprovementPercentage?.toFixed(1)}%
                              </span>
                            </div>
                            <div className="w-full bg-gray-200 rounded-full h-2">
                              <div
                            ...
                            <div className="flex justify-between text-xs items-center">
                              <span className="text-red-700">Poor</span>
                              <span className="font-semibold">
                                {coreWebVitals.desktop.poorPercentage?.toFixed(1)}%
                              </span>
                            </div>
                            <div className="w-full bg-gray-200 rounded-full h-2">
                              <div
                  ...
                  <div className="text-center py-6 text-gray-500">
                    ...
                    <p className="text-sm">{coreWebVitals.error}</p>
                    <Button 
                      variant="outline" 
                      size="sm" 
                      className="mt-3"
                      onClick={() => activeSite && fetchCoreWebVitals(activeSite.siteUrl)}
                    >
                      Retry
                    </Button>
                  </div>
          ...
          {/* Core Web Vitals Detailed Metrics - Always visible */}
          <Card>
            <CardHeader>
              <CardTitle>Web Vitals Detailed Metrics</CardTitle>
              <CardDescription>
                Key performance indicators measured by PageSpeed Insights
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <MetricCard
                  label="LCP - Largest Contentful Paint"
                  mobile={coreWebVitals?.metrics?.lcp?.mobile?.toFixed(1) || '0.0'}
                  desktop={coreWebVitals?.metrics?.lcp?.desktop?.toFixed(1) || '0.0'}
                  unit="s"
                  mobileScore={coreWebVitals?.metrics?.lcp?.mobileScore || 0}
                  desktopScore={coreWebVitals?.metrics?.lcp?.desktopScore || 0}
                  isLoading={isCoreWebVitalsLoading || !coreWebVitals || !coreWebVitals.metrics}
                />
                <MetricCard
                  label="FID - First Input Delay"
                  mobile={coreWebVitals?.metrics?.fid?.mobile ? Math.round(coreWebVitals.metrics.fid.mobile) : '0'}
                  desktop={coreWebVitals?.metrics?.fid?.desktop ? Math.round(coreWebVitals.metrics.fid.desktop) : '0'}
                  unit="ms"
                  mobileScore={coreWebVitals?.metrics?.fid?.mobileScore || 0}
                  desktopScore={coreWebVitals?.metrics?.fid?.desktopScore || 0}
                  isLoading={isCoreWebVitalsLoading || !coreWebVitals || !coreWebVitals.metrics}
                />
                <MetricCard
                  label="CLS - Cumulative Layout Shift"
                  mobile={coreWebVitals?.metrics?.cls?.mobile?.toFixed(2) || '0.00'}
                  desktop={coreWebVitals?.metrics?.cls?.desktop?.toFixed(2) || '0.00'}
                  unit=""
                  mobileScore={coreWebVitals?.metrics?.cls?.mobileScore || 0}
                  desktopScore={coreWebVitals?.metrics?.cls?.desktopScore || 0}
                  isLoading={isCoreWebVitalsLoading || !coreWebVitals || !coreWebVitals.metrics}
                />
                <MetricCard
                  label="FCP - First Contentful Paint"
                  mobile={coreWebVitals?.metrics?.fcp?.mobile?.toFixed(1) || '0.0'}
                  desktop={coreWebVitals?.metrics?.fcp?.desktop?.toFixed(1) || '0.0'}
                  unit="s"
                  mobileScore={coreWebVitals?.metrics?.fcp?.mobileScore || 0}
                  desktopScore={coreWebVitals?.metrics?.fcp?.desktopScore || 0}
                  isLoading={isCoreWebVitalsLoading || !coreWebVitals || !coreWebVitals.metrics}
                />
                <MetricCard
                  label="INP - Interaction to Next Paint"
                  mobile={coreWebVitals?.metrics?.inp?.mobile ? Math.round(coreWebVitals.metrics.inp.mobile) : '0'}
                  desktop={coreWebVitals?.metrics?.inp?.desktop ? Math.round(coreWebVitals.metrics.inp.desktop) : '0'}
                  unit="ms"
                  mobileScore={coreWebVitals?.metrics?.inp?.mobileScore || 0}
                  desktopScore={coreWebVitals?.metrics?.inp?.desktopScore || 0}
                  isLoading={isCoreWebVitalsLoading || !coreWebVitals || !coreWebVitals.metrics}
                />
                <MetricCard
                  label="TTFB - Time to First Byte"
                  mobile={coreWebVitals?.metrics?.ttfb?.mobile ? Math.round(coreWebVitals.metrics.ttfb.mobile) : '0'}
                  desktop={coreWebVitals?.metrics?.ttfb?.desktop ? Math.round(coreWebVitals.metrics.ttfb.desktop) : '0'}
                  unit="ms"
                  mobileScore={coreWebVitals?.metrics?.ttfb?.mobileScore || 0}
                  desktopScore={coreWebVitals?.metrics?.ttfb?.desktopScore || 0}
                  isLoading={isCoreWebVitalsLoading || !coreWebVitals || !coreWebVitals.metrics}
                />
              </div>
              ...
              <div className="mt-6 pt-6 border-t">
                ...
                {isCoreWebVitalsLoading || !coreWebVitals || !coreWebVitals.metrics ? (
                  // Loading state for category scores
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="text-center p-3 bg-gray-50 rounded-lg">
                      ...
                      <p className="text-2xl font-bold text-gray-900">
                        {Math.round(coreWebVitals.mobile?.categoryScores?.accessibility || 0)}
                      </p>
                    </div>
                    <div className="text-center p-3 bg-gray-50 rounded-lg">
                      <p className="text-xs text-gray-600 mb-1">Best Practices</p>
                      <p className="text-2xl font-bold text-gray-900">
                        {Math.round(coreWebVitals.mobile?.categoryScores?.bestPractices || 0)}
                      </p>
                    </div>
                    <div className="text-center p-3 bg-gray-50 rounded-lg">
                      <p className="text-xs text-gray-600 mb-1">SEO</p>
                      <p className="text-2xl font-bold text-gray-900">
                        {Math.round(coreWebVitals.mobile?.categoryScores?.seo || 0)}
                      </p>
                    </div>
                    <div className="text-center p-3 bg-gray-50 rounded-lg">
                      <p className="text-xs text-gray-600 mb-1">Last Updated</p>
                      <p className="text-xs font-medium text-gray-700">
                        {coreWebVitals.lastUpdated ? 
                          new Date(coreWebVitals.lastUpdated).toLocaleDateString() : 
                          'N/A'
                        }
                      </p>
                    </div>
