...
export default function SearchConsoleIndexing() {
  ...
  const fetchSitesAndData = async () => {
    try {
      ...
      // First fetch available sites
      const sitesResponse = await fetch(`${SERVER}/api/v1/seo/sites`, {
        method: "GET",
        credentials: "include",
      ...
      if (
        ...
        const firstSite = sitesResult.sites[0].siteUrl;
        setSiteUrl(firstSite);

        // Now fetch indexing data for first site
        await fetchIndexingData();
      } else {
  ...
  const fetchIndexingData = async () => {
    try {
      ...
      const selectedSite = siteUrl || "YOUR_SITE_URL";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (failed === 0) {
        ...
        setUrls("");
        setTimeout(() => fetchIndexingData(), 2000);
      } else {
        setMessage({
          type: "warning",
